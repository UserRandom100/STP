...
class UserDAO implements DAO {
    ...
    static public function findById(int $id): ?User {
        ...
        $requete = $connexion->prepare(
            "SELECT u.*, r.roleName
            FROM User u 
            JOIN Role r ON u.role = r.roleCode
            WHERE u.id = :id"
        );
    ...
    /**
     * Retourne une liste d'objets filtrée selon le critère donné
     * @param string $filter
     * @return array
     */
    static public function findByDescription(string $filter): array {
        // Cette méthode peut être personnalisée pour gérer un filtre spécifique
        // Par exemple, une recherche par nom ou email
        $tableau = [];
        try {
            $connexion = ConnexionBD::getInstance();
        } catch (Exception $e) {
        ...
        $requete = $connexion->prepare(
            ...
            FROM User u 
            JOIN Role r ON u.role = r.roleCode
            WHERE u.name LIKE :filter OR u.email LIKE :filter"
        );
    ...
    static public function save(object $user): bool {
        ...
        $roleId = $user->getRole() ?? 3; // Par défaut, "User"
